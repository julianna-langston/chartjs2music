...
export const SimpleBarChart: Story = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("User tabs to chart", async () => {
      ...
      await waitFor(() => expect(cc).toHaveTextContent(`Sonified chart titled "Test"`));
      expect(cc).toHaveTextContent("Bar chart.");
      expect(cc).toHaveTextContent(`X is "Year" from 2010 to 2016`);
      expect(cc).toHaveTextContent(`Y is "Count" from 10 to 30`);
    });
  },
};
const data = [
